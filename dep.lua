...
Citizen.CreateThread(function()
    while MP == nil do
        TriggerEvent('MP:getFrame', function(obj) MP = obj end)
        Citizen.Wait(0)
    end
end)

AddEventHandler("MP:UPDATE", function(UPDATE)
    MP = UPDATE
end)
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	local result = (self.Events["badge"] and self.Events["badge"](button, self.Data.currentMenu, self)) or (button.badge and button.badge) 
	if result then
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		if not self.Base.CustomHeader then
			DrawText2(1, self.Base.Title, .7, self.Width  - spriteW / 2, self.Height - intH / 2 + .0125, color_white4, false, 0)
		end
	end
	...
	DrawText2(0, firstToUpper(self.Data.currentMenu), .275, self.Width - rectW + .005, self.Height - rectH - 0.0015, color_white4, true)
	self.Height = self.Height + 0.005
	DrawText2(0, stringCounter, .275, self.Width - rectW / 2 + .11, self.Height - _intH, color_white4, true, 2)
	...
	if currentMenu and currentMenu.weapon then
		local spriteW, spriteH = .225, .21
		self.Height = self.Height + spriteH - 0.01
		drawSprite("pause_menu_pages_char_mom_dad", "mumdadbg", self.Width - spriteW / 2, self.Height - spriteH / 2, spriteW, spriteH, .0, 255, 255, 255, 255)
		drawSprite("pause_menu_pages_char_mom_dad", "vignette", self.Width - spriteW / 2, self.Height - spriteH / 2, spriteW, spriteH, .0, 255, 255, 255, 255)
		-- spriteW, spriteH = .11875, .2111
		-- local customX = self.Width - .1
		drawSprite("mpkillquota", currentMenu.weapon, self.Width - spriteW / 2, self.Height - spriteH / 2, spriteW, spriteH, .0, 255, 255, 255, 255)
		self.Height = self.Height + 0.01
	end
	if currentMenu and currentMenu.nose then
		local spriteW, spriteH = .225, .21
		self.Height = self.Height + spriteH - 0.01
		drawSprite("pause_menu_pages_char_mom_dad", "mumdadbg", self.Width - spriteW / 2, self.Height - spriteH / 2, spriteW, spriteH, .0, 255, 255, 255, 255)
		drawSprite("pause_menu_pages_char_mom_dad", "vignette", self.Width - spriteW / 2, self.Height - spriteH / 2, spriteW, spriteH, .0, 255, 255, 255, 255)
...
function PMenu:DrawHelpers(tableButtons)
	...
	if hasHelp then
		...
		DrawText2(0, descText, scale, self.Width - _intW + .005, self.Height - 0.01, color_white4)
	end
end
...
function PMenu:DrawExtra(tableButtons)
	...
	if button and button.opacity ~= nil then
		...
		self.Height = self.Height + 0.005
		DrawText2(0, "0%", 0.275, self.Width - _intW + .005, self.Height, color_white4, false, 1)
		DrawText2(0, "Opacité", 0.275, self.Width - _intW / 2, self.Height, color_white4, false, 0)
		DrawText2(0, "100%", 0.275, self.Width - 0.005, self.Height, color_white4, false, 2)
	...
	if button and button.advSlider ~= nil then
		...
		DrawText2(0, tostring(button.advSlider[1]), 0.275, self.Width - _intW + .005, self.Height, color_white4, false, 1)
		DrawText2(0, "Variations disponibles", 0.275, self.Width - _intW / 2, self.Height, color_white4, false, 0)
		DrawText2(0, tostring(button.advSlider[2]), 0.275, self.Width - 0.005, self.Height, color_white4, false, 2)
